...
@Component({
  selector: 'app-textbot',
  templateUrl: './textbot.component.html',
  styleUrls: ['./textbot.component.css']
})
export class TextbotComponent implements AfterViewChecked{
  ...
  talkToBot(servicetype: string, usertype: string, userMessage: string){
    const apiUrl = `${this.apiUrl}${encodeURIComponent(userMessage)}`;
      ...
      if(this.serviceType && this.userType){
        ...
        const headers = authToken
          ? new HttpHeaders({ Authorization: `Bearer ${uid}`, servicetype: servicetype, usertype: usertype })
          : new HttpHeaders();
          console.log('Request Headers:', headers); // Log the headers
